...
├── models/                                 # generated by `make train`; not committed (see .gitignore)
...
make train   # regenerates models/*.pkl (not committed to git); required before notebooks 4 & 5
```

The main dependencies are pandas, NumPy, scikit-learn, matplotlib, seaborn, scipy, joblib, and Jupyter. `models/*.pkl` is intentionally not committed to keep the repository small and avoid cross-environment pickle-compatibility issues; `make train` regenerates it locally in a few seconds on the full 500-row dataset.
...
Each notebook reads the previous notebook's output (raw CSV → processed CSV → saved model pipelines), so they're meant to be run in sequence on a fresh checkout. The same pipeline is also runnable as scripts via `make all` (or `python -m src.run_pipeline`). Model artifacts under `models/` are not committed to git; run `make train` (or `make all`) once before opening notebooks 4 or 5 so `serve_win_probability_model.pkl` and `model_features.pkl` exist on disk.

You can also query the recommendation step directly for an arbitrary match context via the CLI, e.g.:

```bash
python -m src.run_pipeline recommend \
  --server-score 10 --receiver-score 10 --game-state deuce \
  --opponent-skill-level advanced --opponent-style looper --side backhand_side \
  --top-n 10 --min-reliability 0.5
```

Any flag you omit falls back to the built-in example context. `--min-reliability` controls the minimum `combo_reliability` (0.0–1.0) a candidate must have to appear in the ranked output; the default (`0.5`, i.e. 15+ historical attempts) matches the notebook's "High Confidence Filter" — pass `0.0` to see the unfiltered ranking.
...
- Apply the default minimum-reliability filter (already the default in `src/run_pipeline.py`'s recommend step) to the recommendation notebook's main ranked output too, rather than as a separate manual filter cell.
